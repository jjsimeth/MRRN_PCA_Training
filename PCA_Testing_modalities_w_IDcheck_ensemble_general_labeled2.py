...
if opt.test_case.lower()=='prostate158_ann1_adc':
    print('Using prostate158 dataset with ADC annotations')
    ...
    val_files = find_common_files(valfolders, custom_names,pattern=r'^(00[0-9]|01[0-9])(?:_.*)?\.nii\.gz$') #Edited regexs to include any file with three consecutive numbers
...
if opt.test_case.lower()=='prostate158_ann2_adc':
    print('Using prostate158 dataset with ADC annotations')
    valpath_adcpath = os.path.join(datadir,'MR_Prostate158_Train','Images','ADC')
    valpath_t2path = os.path.join(datadir,'MR_Prostate158_Train','Images','T2w')
    valpath_masks = os.path.join(datadir,'MR_Prostate158_Train','Masks','DIL')
...
if opt.test_case.lower()=='prostate158_t2':
...
elif opt.test_case.lower()=='prostatex' or opt.test_case.lower()=='prostatex_adc' :
    print('Using prostatex dataset with ADC annotations')  
...
elif opt.test_case.lower()=='prostatex_t2' :  
    print('Using prostatex dataset with T2 annotations')
    valpath_adcpath = os.path.join(datadir,'MR_ProstateX','Images','ADC')
    valpath_t2path = os.path.join(datadir,'MR_ProstateX','Images','T2w')
    #valpath_masks = os.path.join(datadir,'MR_ProstateX','Masks','ADC_GS')
    valpath_masks = os.path.join(datadir,'MR_ProstateX','Masks','T2w_GS')
    ...
    valpath_masks=[]
    for fname in types:
       valpath_masks.extend(glob(os.path.join(valpath_root, fname)))
    ...
    val_files = [{"t2w": t2w, "seg": seg, "img" : img, "prost": prost} for t2w, seg, img, prost in zip(valpath_t2path, valpath_masks, valpath_adcpath, valpath_prostate_masks )] #last  n_val to validation
