...
impath = os.path.join(root_dir,'training_data_v2') #load weights
valpath = os.path.join(root_dir,'validation_data_v2') #load weights
...
types = ('ProstateX*_ep2d_diff_*.nii.gz', '*_ivim_adc_perffrac.nii') # the tuple of file types
images_perf=[]
for fname in types:
   images_perf.extend(glob(os.path.join(impath, fname)))
images_perf = sorted(images_perf)
...
types = ('ProstateX*_t2_tse*.nii.gz', '*_ivim_adc_perffrac.nii') # the tuple of file types
val_images_perf=[]
for fname in types:
   val_images_perf.extend(glob(os.path.join(valpath, fname)))
val_images_perf = sorted(val_images_perf)



print(f"impath: {impath}")
print(f"valpath: {valpath}")
...
# n_val=3
# n_train=7
train_files = [{"img": img, "seg": seg, "t2w": t2w, "perf": perf} for img, seg, t2w, perf in zip(images, segs, images_t2w, images_perf)] #first n_train to training
val_files = [{"img": img, "seg": seg, "t2w": t2w, "perf": perf} for img, seg, t2w, perf in zip(val_images, val_segs, val_images_t2w, val_images_perf)] #last  n_val to validation
...
train_transforms = Compose(
    [
            LoadImaged(keys=["img", "t2w", "perf", "seg"]),
            EnsureChannelFirstd(keys=["img", "t2w", "perf", "seg"]),
            Orientationd(keys=["img", "t2w","perf", "seg"], axcodes="RAS"),
            ...
            #                  key_dst="t2w",
            #                  mode="nearest"),
            ResampleToMatchd(keys=["img","perf","seg"],
                             key_dst="t2w",
                             mode=("bilinear","bilinear", "nearest")),
            Spacingd(keys=["img", "t2w", "perf", "seg"],
            ...
            RandAdjustContrastd(keys=["perf"], prob=0.05),
            RandHistogramShiftd(keys=["perf"], prob=0.05),
            RandBiasFieldd(keys=["perf"], prob=0.05),
            
            RandGaussianNoised(keys=["img", "t2w","perf"], prob=0.15),
            RandGaussianSmoothd(keys=["img", "t2w", "perf"], prob=0.15),
            #RandSimulateLowResolutiond(keys=["img", "t2w"], prob=0.15),
            
            ScaleIntensityRangePercentilesd(keys=["img", "t2w","perf"], lower=0, upper=99, b_min=-1.0, b_max=1.0, clip=True),
            # RandCropByPosNegLabeld(["img", 'lbl'], "lbl", spatial_size=(32, 32)),
            
            CenterSpatialCropd(keys=["img","t2w","perf","seg"], roi_size=[256, 256,-1]),
            ...
            CropForegroundd(keys=["img", "t2w","perf", "seg"], source_key="seg", margin=[96, 96, opt.extra_neg_slices + (opt.nslices - 1) / 2]),
            

            RandFlipd(keys=["img", "t2w","perf", "seg"], prob=0.5),
            RandAxisFlipd(keys=["img", "t2w","perf", "seg"], prob=0.25),
            
            RandRotated(keys=["img", "t2w","perf", "seg"], prob=1.0, range_x=1.0, range_y=0.0, range_z=0.0, mode=("bilinear", "bilinear","bilinear", "nearest")),
            ...
            RandSpatialCropd(keys=["img", "t2w","perf", "seg"], roi_size=(128, 128, opt.nslices), random_size=False,random_center =True),
            Transposed(keys=["img", "seg", "t2w", "perf"], indices=[3, 2, 1, 0]),
            SqueezeDimd(keys=["img", "t2w","perf", "seg"], dim=-1),
            EnsureTyped(keys=["img", "t2w","perf", "seg"]),
...
def debug_transforms(data, transforms, name="Transforms"):
    print(f"Before {name}:")
    ...
    try:
        transformed_data = [transforms(item) for item in data]
...
# Wrap train_transforms for debugging
debug_train_files = debug_transforms(train_files, train_transforms, name="Train Transforms")
...
# Wrap val_transforms for debugging
debug_val_files = debug_transforms(val_files, val_transforms, name="Validation Transforms")
...
# Process first 10 batches from the training loader
for idx, data in enumerate(train_loader, 10):
    # Create unique folder for each case
    case_path = os.path.join(dest_path, f'case_{idx+1}')
    ...
    print(data["perf"])
    print("------------")
    print(f"Case {idx+1} shapes:", data["img"].shape, data["seg"].shape, data["t2w"].shape, data["perf"].shape)
    
    # Save each modality (t2w, ADC map, and segmentation mask)
    for modality, filename in [("t2w", "t2w"), ("img", "adc"), ("seg", "SEG"), ("perf","perf")]:
    #for modality, filename in [("img", "adc"), ("seg", "SEG")]:
        sitk.WriteImage(
            sitk.GetImageFromArray(data[modality].cpu()[0,:,:,:]),  # Extract 3D volume from batch
            os.path.join(case_path, f'TRAINING_DEBUG_{filename}.nii.gz')
        )
...
for epoch in range(num_epochs+1):
    ...
    for batch_data in train_loader:
        ...
        adc, labels, t2w, perf = batch_data["img"].to(device), batch_data["seg"].to(device), batch_data["t2w"].to(device), batch_data["perf"].to(device)
    ...
    if (epoch%opt.display_freq)==0:
        ...
        with torch.no_grad(): # no grade calculation 
            ...
            for val_data in val_loader:
                step += 1
                adc, label_val,t2w, perf = val_data["img"].to(device), val_data["seg"].to(device), val_data["t2w"].to(device) , val_data["perf"].to(device)
                ...
                #label_data = label_val_vol.to(seg.device)
                ...
                #dice_2D_temp = (2. * intersection + smooth) / (torch.sum(seg_flat) + torch.sum(gt_flat) + smooth)
            ...
            # Calculate average
            #avg_dice_2D_gpu = torch.mean(dice_2D)
            #dice_2D = avg_dice_2D_gpu.cpu().item()
            ...
            if dice_2D>best_dice:
                    ...
                    for vol_data in val_loader:
                        im_iter += 1
                        adc, label_val,t2w, perf = vol_data["img"].to(device), vol_data["seg"].to(device), vol_data["t2w"].to(device), vol_data["perf"].to(device)                        
                        ...
                        sitk.WriteImage(sitk.GetImageFromArray(perf.cpu()),  os.path.join(results_path,'val%i_perf.nii.gz' % im_iter))
