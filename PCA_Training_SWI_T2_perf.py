...
for fname in types:
   images_perf.extend(glob(os.path.join(impath, fname)))
val_images_perf = sorted(images_perf)
...
# n_val=3
# n_train=7
train_files = [{"img": img, "seg": seg, "t2w": t2w} for img, seg, t2w in zip(images_perf, segs, images_t2w)] #first n_train to training
val_files = [{"img": img, "seg": seg, "t2w": t2w} for img, seg, t2w in zip(val_images_perf, val_segs, val_images_t2w)] #last  n_val to validation
...
for idx, data in enumerate(train_loader, 10):
    ...
    print(f"Case {idx+1} shapes:", data["img"].shape, data["seg"].shape, data["t2w"].shape)
    
    # Save each modality (t2w, ADC map, and segmentation mask)
    for modality, filename in [("t2w", "t2w"), ("img", "adc"), ("seg", "SEG")]:
