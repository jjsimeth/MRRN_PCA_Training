...
valpath = os.path.join(root_dir,'testing_data_v2') #load weights
...
for fname in types:
   val_images_t2w.extend(glob(os.path.join(valpath, fname)))
val_images_perf = sorted(val_images_t2w)
...
val_transforms = Compose(
    [
        ...
        ScaleIntensityRangePercentilesd(keys=["perf"],lower=0,upper=99,b_min=-1.0,b_max=1.0,clip=True),
...
print("first patch's shape: ", check_data["img"].shape, check_data["seg"].shape, check_data["t2w"].shape, check_data["perf"].shape)
...
sitk.WriteImage(sitk.GetImageFromArray(check_data["perf"].cpu()[:,:,:,:]),  os.path.join(dest_path,'TRAINING_DEBUG_perf.nii.gz'))
...
with torch.no_grad(): # no grade calculation 
    ...
    for val_data in val_loader:
        ...
        perf_name=perf.meta['filename_or_obj'][0].split('/')[-1]
