...
class Incre_MRRN_PreTrained_MultiModal(nn.Module):
    #def __init__(self, n_channels, n_classes,opt):
    def __init__(self, n_channels, n_classes,deeplayer=0):
        super(Incre_MRRN_PreTrained_MultiModal, self).__init__()
        
        pretrained_channels=5
        self.deeplayer=deeplayer
        
        self.ChannelAdapting_CNN_block0=CNN_block(n_channels,pretrained_channels)
        
        #self.tsne=opt.tsne
        self.CNN_block1=CNN_block(pretrained_channels,32)        
    ...
    def forward (self,x):
        ...
        x0=self.ChannelAdapting_CNN_block0(x)
        x1= self.CNN_block1(x0)   
        x2= self.RU1(x1)
        x3= self.RU2(x2)
        x4= self.RU3(x3)
        # Three RU 
