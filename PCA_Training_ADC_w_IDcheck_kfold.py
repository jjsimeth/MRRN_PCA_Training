...
def kfold_split(dataset,stratification_data=None, k=5, seed=42):
    ...
    # Group indices by extracted ID
    id_to_indices = defaultdict(list)
    id_to_group = defaultdict(list)
    for idx, sample in enumerate(dataset):
        file_id = extract_id(sample)
        if file_id:
            strat_group=stratification_data[idx]
            file_id=file_id+'_G%i' %strat_group
            id_to_indices[file_id].append(idx)
            id_to_group[file_id]=strat_group
    
    # Get unique IDs and shuffle consistently
    ...
    id_groups = [id_to_group[file_id] for file_id in unique_ids]
    
    #id_groups = [group for id in unique_ids for group in id_to_group[id]]
    # Create KFold object
    #kf = KFold(n_splits=k, shuffle=False)  # Already shuffled manually for consistency
    kf = StratifiedKFold(n_splits=k, shuffle=False)  # Already shuffled manually for consistency
    
    
    #Define pt characterisitcs
    ...
    folds = []
    for fold_idx, (train_idx, val_idx) in enumerate(kf.split(unique_ids,y=id_groups)):
        train_ids = [unique_ids[i] for i in train_idx]
        val_ids = [unique_ids[i] for i in val_idx]
        
        train_indices = [idx for id in train_ids for idx in id_to_indices[id]]
        val_indices = [idx for id in val_ids for idx in id_to_indices[id]]
        ...
        # Save IDs to file
        with open(f'fold_{fold_idx+1}_ids.txt', 'w') as f:
            f.write(f"Fold {fold_idx+1}\n")
            f.write("Train IDs: " + ", ".join(train_ids) + "\n")
            f.write("Validation IDs: " + ", ".join(val_ids) + "\n")
...
train_transforms = Compose(
    [
            ...
            OneOf(transforms=[
                ...
                RandSimulateLowResolutiond(keys=["img"], prob=0.5),
                RandGaussianNoised(keys=["img"], prob=0.5),
                RandGaussianSmoothd(keys=["img"], prob=0.5),
                RandGaussianSharpend(keys=["t2w"], prob=0.5),
                ]),
                    
            OneOf(transforms=[
                RandAdjustContrastd(keys=["t2w"], prob=0.5),
                RandHistogramShiftd(keys=["t2w"], prob=0.5),
                RandBiasFieldd(keys=["t2w"], prob=0.5),
                RandSimulateLowResolutiond(keys=["t2w"], prob=0.5),
                RandGaussianNoised(keys=["t2w"], prob=0.5),
                RandGaussianSmoothd(keys=["t2w"], prob=0.5),
                RandGaussianSharpend(keys=["t2w"], prob=0.5),
                ]),
