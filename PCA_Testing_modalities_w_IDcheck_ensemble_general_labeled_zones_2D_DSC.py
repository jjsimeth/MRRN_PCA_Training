...
def getDICE_2D(pred, ref):
    #Binarize inputs
    pred = (pred > 0.5).astype(np.float32)
    ref = (ref > 0.5).astype(np.float32)
    
    #Volumetric DSC
    vol_intersection = np.sum(pred * ref)
    vol_sum = np.sum(pred) + np.sum(ref)
    vol_DSC = (2.0 * vol_intersection / vol_sum) if vol_sum > 0 else 1.0
    
    #2D DSC per slice
    DSC_slices = []
    for i in range(ref.shape[-1]):  # iterate over slices along last axis
        ref_slice = ref[..., i]
        pred_slice = pred[..., i]
        
        if np.sum(ref_slice) > 0:  # only compute if ref has foreground
            intersection = np.sum(pred_slice * ref_slice)
            union = np.sum(pred_slice) + np.sum(ref_slice)
            dsc = (2.0 * intersection / union)
            DSC_slices.append(dsc)
    
    return vol_DSC, DSC_slices


def process_with_csv(img_name,gt_name,model_name,dataset_name, seg_filtered, gtv, adc, spacing_mm, fd_results, csv_path=None,zones=None):
    # Optional CSV load
    df = None
    ...
    for i, ilesion in enumerate(labels):  # i = index into metric arrays, ilesion = actual label ID
        ...
        if csv_info.empty:
            ...
            #print(ilesion)
            izonein=lesion_data.get("Zone", izone).values[0]
            if izonein=='nan':
                izonein=izone
...
def compute_metrics_per_lesion(labeled_gtv, labeled_pred,adc, spacing_mm):
    ...
    for label_gt in labels_gt:
        ...
        for pred_label in overlapping_pred_labels:
            ...
            vol_DSC, DSC2D = getDICE_2D(pred_mask, gt_mask)
            dsc= np.mean(DSC2D)
            if dsc > best_dsc:
                best_dsc = dsc
  ...
  #filter out small unconnected segs
    ...
    for ilabel in range(0,ncomponents_seg):
        ...
        for jlabel in range(0,ncomponents_gtv):
            ...
            vol_DSC, DSC2D = getDICE_2D(pred, target)
            
            DSC_array[ilabel,jlabel] = np.mean(DSC2D)
...
seg_path=os.path.join(resultsdir,opt.name,opt.test_case)
...
#     valpath_masks = os.path.join(datadir,'MR_Prostate158','Masks','DIL')
#     valpath_masks2 = os.path.join(datadir,'MR_Prostate158','Masks','DIL2')
#     valpath_prostate_masks = os.path.join(datadir,'MR_Prostate158','Masks','Prostate2')
#     # valpath_prostate_masks = os.path.join(datadir,'MR_Prostate158_Train','Masks','TZ')
#     # valpath_prostate_masks = os.path.join(datadir,'MR_Prostate158_Train','Masks','PZ')
...
# val_transforms_p158 = Compose(
#     [
#         LoadImaged(keys=["img","seg","prost", "t2w","TZ","PZ"]),
#         EnsureChannelFirstd(keys=["img", "seg","prost", "t2w","TZ","PZ"]),
#         Orientationd(keys=["img","seg","prost", "t2w","TZ","PZ"], axcodes="RAS"),     
        
#         ResampleToMatchd(keys=["t2w","seg","prost","TZ","PZ"],
#                               key_dst="img",
#                               mode=("bilinear", "nearest", "nearest")),
#         Spacingd(keys=["img", "seg","prost", "t2w","TZ","PZ"],
#                       pixdim=(PD_in[0], PD_in[1], PD_in[2]),
#                       mode=("bilinear", "nearest", "nearest","bilinear")),
        
#         ScaleIntensityRangePercentilesd(keys=["img", "t2w"],lower=0,upper=99,b_min=-1.0,b_max=1.0,clip=True,channel_wise=True),
#         CropForegroundd(keys=["img","seg","prost", "t2w","TZ","PZ"], source_key= "prost", margin=[64,64,opt.extra_neg_slices + (opt.nslices - 1) / 2]),
        
        
#         #RandSpatialCropd(keys=["img","t2w","seg"], roi_size=(128, 128, opt.nslices),random_size=False),,
#         CenterSpatialCropd(keys=["img","t2w", "prost","seg","TZ","PZ"], roi_size=[128, 128,-1]),
#         EnsureTyped(keys=["img", "seg","prost", "t2w","TZ","PZ"]),
#         AsDiscreted(keys=["prost","seg","TZ","PZ"], rounding="torchrounding"),
#     ]
#   )
...
# post_transforms_p158 = Compose([
#         EnsureTyped(keys=["pred","seg","prost","img","PZ","TZ"]),
#         Invertd(
#             keys=["pred","seg","prost","img","PZ","TZ"],
#             transform=val_transforms,
#             orig_keys="img",
#             meta_keys=["pred_meta_dict","pred_meta_dict","pred_meta_dict","image_meta_dict","pred_meta_dict","pred_meta_dict"],
#             orig_meta_keys="image_meta_dict",
#             meta_key_postfix="meta_dict",
#             nearest_interp=True,
#             to_tensor=True,
#         ),
#         AsDiscreted(keys=["pred","seg","prost","img","PZ","TZ"], argmax=False),

#     ])

# if opt.test_case.lower()=='prostate158':
#     post_transforms=post_transforms_p158
#     val_transforms=val_transforms_p158
...
with torch.no_grad(): # no grade calculation 
    ...
    for val_data in val_loader:
        ...
        print('******************************************************')
        #val_inputs=val_inputs.permute(0,1,3,2,4)
        ...
        if adc.numel() != 0:
            ...
            with torch.no_grad():
                with autocast(enabled=True):
                    for model in models:
                        ...
                        # pred = sliding_window_inference(
                        #     val_inputs.permute(0,1,3,2,4),                       # e.g. (B,C,H,W,D)
                        #     (128, 128, opt.nslices),         # ROI size
                        #     1,                               # sw_batch_size
                        #     model.netSeg_A,
                        #     overlap=0.80,
                        ...
                        #     options=opt
                        # )
                        # predictions.append(pred.permute(0,1,3,2,4))
            ...
            zones=None
            if opt.test_case.lower()=='prostate158':
                zones=np.squeeze(np.array(prostate))
            ...
            if  np.sum(gtv)>0:
                ...
                dilations=0
                max_dilations=100
                prostate=ndimage.binary_dilation(prostate,iterations=3).astype(prostate.dtype)
                while np.sum(prostate*(gtv>0.5))<(0.9*np.sum(gtv>0.5)) and dilations<max_dilations:
                    dilations+=1
                    prostate=ndimage.binary_dilation(prostate,iterations=1).astype(prostate.dtype)
                
                if dilations==max_dilations:
                    print('prostate mask issue - > minimal/no overlap - skip prostate masking')
                ...
                # zones=None
                # if opt.test_case.lower()=='prostate158':
                #     PZ = np.squeeze(np.array(from_engine(["PZ"])(val_data)[0]))
                #     TZ = np.squeeze(np.array(from_engine(["TZ"])(val_data)[0]))
                #     zones=np.zeros_like(gtv)
                #     zones(PZ>0)==1.0
                #     zones(TZ>0)==1.0
                
                
                
                DSC, FP, hd95, gt_vol, pred_vol,gt_adc, pred_adc, labels=process_with_csv(img_name,gtv_name,opt.name,opt.test_case, seg_filtered, gtv, adc_val, spacing_mm, fd_results, csv_path,zones=zones)
