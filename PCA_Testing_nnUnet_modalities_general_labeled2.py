...
def process_with_csv(img_name,gt_name,model_name,dataset_name, model_arch_name, seg_filtered, gtv, adc, spacing_mm, fd_results, csv_path=None):
    # Optional CSV load
    df = None
    if csv_path is not None:
        try:
            ...
            print('something went wrong trying to read the xlsx or the csv, consider trying the other format')
    ...
    DSC, FP, hd95, auc, gt_vol, pred_vol,gt_adc, pred_adc, labels = lesion_eval(
    ...
    print_metrics(DSC, hd95, dice_3D_temp, auc, gt_vol, pred_vol)
    ...
    if df is not None:
        ...
        if img_id:
            try:
                ...
                print(f"Something went wrong while parsing the patid: {e}")
                img_id = None
    ...
    for i, ilesion in enumerate(labels):  # i = index into metric arrays, ilesion = actual label ID
        if csv_info.empty:
            ...
            lesion_row = {
                ...
                "ROI_Volume": np.nan,
                "Scanner": np.nan,
                "Voxel_size_ADC": np.nan,
                "Voxel_size_T2": np.nan,
            }
        else:
            ...
            if not lesion_data.empty:
                lesion_row = {
                    ...
                    "Voxel_size_ADC": lesion_data.get("Voxel_size_ADC", pd.Series([np.nan])).values[0],
                    "Voxel_size_T2": lesion_data.get("Voxel_size_T2", pd.Series([np.nan])).values[0],
                }
            else:
                lesion_row = {
                    "Gleason_Grade_Group": np.nan,
        ...
        fd_results.write(
            f"{model_name},{dataset_name},{model_arch_name},{img_name},{gt_name},{ilesion},{DSC[i]},{dice_3D_temp},{hd95[i]},{auc},"
            ...
            f"{lesion_row['ROI_Volume']},{lesion_row['Scanner']},{lesion_row['Voxel_size_ADC']}, {lesion_row['Voxel_size_T2']}\n"
        )
    ...
    return DSC, FP, hd95, auc, gt_vol, pred_vol,gt_adc, pred_adc, labels
        
def print_metrics(DSC, hd95, dice_3D_temp, auc, gt_vol, pred_vol):
...
def calculate_auc_scores(seg, labeled_gtv):
    """
    Calculate  overall n AUC scores using sklearn's roc_auc_score implementation.
    
    Returns:
        overall_auc: Single AUC for a case with all lesions (could be disconeccted) vs background
    """        
        
    seg_flat = seg.flatten()
    
    # Calculate overall AUC (any lesion vs background)
    try:
        labeled_gtv_binary = (labeled_gtv > 0).astype(int)
        labeled_gtv_flat = labeled_gtv_binary.flatten()
        
        if len(np.unique(labeled_gtv_flat)) > 1:
            overall_auc = roc_auc_score(labeled_gtv_flat, seg_flat)
        else:
...
def lesion_eval(seg,gtv,adc, spacing_mm, connected_component_analysis=True):
  #filter out small unconnected segs
    ...
    auc = calculate_auc_scores(seg, labeled_gtv)
    
    #print(labels_seg)
    print('  lesions/preds, FP: %i/%i, %i' % (ncomponents_gtv,np.size(np.unique(labeled_seg[labeled_seg != 0])),FD))
    return  DSC, FD, hd95, auc, gt_vol, pred_vol,gt_adc, pred_adc,labels  
...
model_arch_name = 'nnUnet_softmax'
...
resultspaths= [r'/lila/data/deasy/Josiah_data/Prostate/results/NKI_MSK' , r'/gpfs/home1/rbosschaert/Results_NKI_MSK']
...
# )
fd_results.write(
    "Model,Dataset,Model Arch,Filename,Lesion Filename,LesionN,Lesion DSC,Whole Volume DSC,hd95 (mm), AUC,"
    "GT Vol Calculated (mL),Pred Vol (mL),GT ADC,Pred ADC,False Positives,"
    "Gleason_Grade_Group,PIRADS,ISUP,Zone,Clinically_significant,"
    "ROI_Volume,Scanner,Voxel_size_ADC,Voxel_size_T2 \n"
)
...
csv_path=None
custom_names = ["img", "seg", "t2w","prost", "nnu_softmax"]

#==========================================================================

if opt.test_case.lower()=='prostate158_adc_ann1_adc' or opt.test_case.lower()== 'prostate158_adc_ann1_t2' or opt.test_case.lower()=='prostate158_adc_ann1_t2adc' or opt.test_case.lower()=='prostate158_adc_ann1_adct2' or opt.test_case.lower()=='prostate158_adc_ann1_adc_t2' or opt.test_case.lower()=='prostate158_adc_ann1_t2_adc' :
    print('Using prostate158 dataset with ADC annotations of annotator 1')
    ...
    #valpath_masks = os.path.join(datadir,'MR_Prostate158_Train','Masks','T2w_DIL')
    #valpath_prostate_masks = os.path.join(datadir,'MR_Prostate158_Train','Masks','Prostate')
    
    #Load softmax and corresponding GT and pros
    nnunet_path = os.path.join(datadir ,'MR_Prostate158_Train','Nnunet')
    if opt.test_case.lower()=='prostate158_adc_ann1_adc':
      nnunet_path = os.path.join(nnunet_path,'ADC')
    elif opt.test_case.lower()=='prostate158_adc_ann1_t2':
      nnunet_path = os.path.join(nnunet_path,'T2')
    elif opt.test_case.lower()=='prostate158_adc_ann1_t2adc' or opt.test_case.lower()=='prostate158_adc_ann1_adct2' or opt.test_case.lower()=='prostate158_adc_ann1_adc_t2' or opt.test_case.lower()=='prostate158_adc_ann1_t2_adc' :
      nnunet_path = os.path.join(nnunet_path,'T2ADC')
    else:
      print('Did not get one of the expected input variables: prostate158_adc_ann1_adc, prostate158_adc_ann1_t2, prostate158_adc_ann1_t2adc')
      
    valpath_prostate_masks = os.path.join(nnunet_path,'Prostate')
    valpath_masks = os.path.join(nnunet_path ,'Labels_ADC_1')
    nnunet_pred_fold = os.path.join(nnunet_path ,'Softmax')
    
    impath=nnunet_pred_fold
    
    valfolders = [valpath_adcpath, valpath_masks,valpath_t2path,valpath_prostate_masks, nnunet_pred_fold]  # Replace with actual paths
    custom_names = ["img", "seg", "t2w","prost","softmax"]
    
    val_files = find_common_files(valfolders, custom_names,pattern=r'^(00[0-9]|01[0-9])(?:_.*)?\.nii\.gz$') 
    
    #csv_path=os.path.join(datadir,'MR_ProstateX','ADC_lesions.csv')

elif opt.test_case.lower()=='prostate158_adc_ann2_adc' or opt.test_case.lower()=='prostate158_adc_ann2_t2' or opt.test_case.lower()=='prostate158_adc_ann2_t2adc' or opt.test_case.lower()=='prostate158_adc_ann2_adct2' or opt.test_case.lower()=='prostate158_adc_ann2_adc_t2' or opt.test_case.lower()=='prostate158_adc_ann2_t2_adc' :
    print('Using prostate158 dataset with ADC annotations of annotator 2')
    valpath_adcpath = os.path.join(datadir,'MR_Prostate158_Train','Images','ADC')
    valpath_t2path = os.path.join(datadir,'MR_Prostate158_Train','Images','T2w')
    #valpath_masks = os.path.join(datadir,'MR_Prostate158_Train','Masks','DIL')
    #valpath_masks2 = os.path.join(datadir,'MR_Prostate158_Train','Masks','DIL2')
    ...
    #Load softmax and corresponding GT and pros
    nnunet_path = os.path.join(datadir ,'MR_Prostate158_Train','Nnunet')
    if opt.test_case.lower()=='prostate158_adc_ann2_adc':
      nnunet_path = os.path.join(nnunet_path,'ADC')
    elif opt.test_case.lower()=='prostate158_adc_ann2_t2':
      nnunet_path = os.path.join(nnunet_path,'T2')
    elif opt.test_case.lower()=='prostate158_adc_ann2_t2adc' or opt.test_case.lower()=='prostate158_adc_ann2_adct2' or opt.test_case.lower()=='prostate158_adc_ann2_adc_t2' or opt.test_case.lower()=='prostate158_adc_ann2_t2_adc' :
      nnunet_path = os.path.join(nnunet_path,'T2ADC')
    else:
      print('Did not get one of the expected input variables: prostate158_adc_ann1_adc, prostate158_adc_ann1_t2, prostate158_adc_ann1_t2adc')
      
    valpath_prostate_masks = os.path.join(nnunet_path,'Prostate')
    valpath_masks = os.path.join(nnunet_path ,'Labels_ADC_2')
    nnunet_pred_fold = os.path.join(nnunet_path ,'Softmax')
...
elif opt.test_case.lower()=='prostate158_t2_ann_adc' or opt.test_case.lower()=='prostate158_t2_ann_t2' or opt.test_case.lower()=='prostate158_t2_ann_t2adc' or opt.test_case.lower()=='prostate158_t2_ann_adct2' or opt.test_case.lower()=='prostate158_t2_ann_adc_t2' or opt.test_case.lower()=='prostate158_t2_ann_t2_adc' : 
    print('Using prostate158 dataset with T2 annotations')
    valpath_adcpath = os.path.join(datadir,'MR_Prostate158_Train','Images','ADC')
    valpath_t2path = os.path.join(datadir,'MR_Prostate158_Train','Images','T2w')
    #valpath_masks = os.path.join(datadir,'MR_Prostate158_Train','Masks','DIL')
    #valpath_masks2 = os.path.join(datadir,'MR_Prostate158_Train','Masks','DIL2')
    ...
    #Load softmax and corresponding GT and pros
    nnunet_path = os.path.join(datadir ,'MR_Prostate158_Train','Nnunet')
    if opt.test_case.lower()=='prostate158_t2_ann_adc':
      nnunet_path = os.path.join(nnunet_path,'ADC')
    elif opt.test_case.lower()=='prostate158_t2_ann_t2':
      nnunet_path = os.path.join(nnunet_path,'T2')
    elif opt.test_case.lower()=='prostate158_t2_ann_t2adc' or opt.test_case.lower()=='prostate158_t2_ann_adct2' or opt.test_case.lower()=='prostate158_t2_ann_adc_t2' or opt.test_case.lower()=='prostate158_t2_ann_t2_adc' :
      nnunet_path = os.path.join(nnunet_path,'T2ADC')
    else:
      print('Did not get one of the expected input variables: prostate158_t2_ann_adc, prostate158_t2_ann_t2, prostate158_t2_ann_t2adc')
      
    valpath_prostate_masks = os.path.join(nnunet_path,'Prostate')
    valpath_masks = os.path.join(nnunet_path ,'Labels_T2_1')
    nnunet_pred_fold = os.path.join(nnunet_path ,'Softmax')
    ...
    #csv_path=os.path.join(datadir,'MR_ProstateX','T2_lesions.csv')
    
elif opt.test_case.lower()=='prostatex_adc_ann_adc' or opt.test_case.lower()=='prostatex_adc_ann_t2' or opt.test_case.lower()=='prostatex_adc_ann_t2adc' or opt.test_case.lower()=='prostatex_adc_ann_adct2' or opt.test_case.lower()=='prostatex_adc_ann_adc_t2' or opt.test_case.lower()=='prostatex_adc_ann_t2_adc' : 
    print('Using prostatex dataset with ADC resolution and ADC annotation')
    valpath_adcpath = os.path.join(datadir,'MR_ProstateX','Images','ADC')
    valpath_t2path = os.path.join(datadir,'MR_ProstateX','Images','T2w')

    #Load softmax and corresponding GT and pros
    
    if opt.test_case.lower()=='prostatex_adc_ann_adc':
      nnunet_path = os.path.join(datadir ,'MR_ProstateX','Nnunet','ADCres', 'ADC')
    elif opt.test_case.lower()=='prostatex_adc_ann_t2':
      nnunet_path = os.path.join(datadir ,'MR_ProstateX','Nnunet','ADCres', 'T2')
    elif opt.test_case.lower()=='prostatex_adc_ann_t2adc' or opt.test_case.lower()=='prostatex_adc_ann_adct2' or opt.test_case.lower()=='prostatex_adc_ann_adc_t2' or opt.test_case.lower()=='prostatex_adc_ann_t2_adc' :
      nnunet_path = os.path.join(datadir ,'MR_ProstateX','Nnunet','ADCres', 'T2ADC')
    else:
      print('Did not get one of the expected input variables: prostatex_adc_ann_adc, prostatex_adc_ann_t2, prostatex_adc_ann_t2adc')
      
    nnunet_pred_fold = os.path.join(nnunet_path,'Softmax')
    valpath_masks = os.path.join(nnunet_path,'Labels')
    valpath_prostate_masks = os.path.join(nnunet_path,'Prostate')
...
elif opt.test_case.lower()=='prostatex_t2_ann_adc' or opt.test_case.lower()=='prostatex_t2_ann_t2' or opt.test_case.lower()=='prostatex_t2_ann_t2adc' or opt.test_case.lower()=='prostatex_t2_ann_adct2' or opt.test_case.lower()=='prostatex_t2_ann_adc_t2' or opt.test_case.lower()=='prostatex_t2_ann_t2_adc' : 
    print('Using prostatex dataset with T2 resolution and T2 annotation')
    valpath_adcpath = os.path.join(datadir,'MR_ProstateX','Images','ADC')
    valpath_t2path = os.path.join(datadir,'MR_ProstateX','Images','T2w')

    #Load softmax and corresponding GT and pros
    
    if opt.test_case.lower()=='prostatex_t2_ann_adc':
      nnunet_path = os.path.join(datadir ,'MR_ProstateX','Nnunet','T2res', 'ADC')
    elif opt.test_case.lower()=='prostatex_t2_ann_t2':
      nnunet_path = os.path.join(datadir ,'MR_ProstateX','Nnunet','T2res', 'T2')
    elif opt.test_case.lower()=='prostatex_t2_ann_t2adc' or opt.test_case.lower()=='prostatex_t2_ann_adct2' or opt.test_case.lower()=='prostatex_t2_ann_adc_t2' or opt.test_case.lower()=='prostatex_t2_ann_t2_adc':
      nnunet_path = os.path.join(datadir ,'MR_ProstateX','Nnunet','T2res', 'T2ADC')
    else:
      print('Did not get one of the expected input variables: prostatex_t2_ann_adc, prostatex_t2_ann_t2, prostatex_t2_ann_t2adc')
...
elif opt.test_case.lower()=='nki_adc' or opt.test_case.lower()=='nki_t2' or opt.test_case.lower()=='nki_t2adc' or opt.test_case.lower()=='nki_adct2' or opt.test_case.lower()=='nki_adc_t2' or opt.test_case.lower()=='nki_t2_adc' : 
    print('Using NKI dataset ') 
    valpath_root = os.path.join(datadir,'testing_data_v3')

    types = ('any_other_relevant_files.nii','*_ivim_adc.nii') 
    valpath_adcpath=[]
    for fname in types:
       valpath_adcpath.extend(glob(os.path.join(valpath_root, fname)))
    valpath_adcpath = sorted(valpath_adcpath)
    
    types = ('any_other_relevant_files.nii','*_tt2.nii') 
    valpath_t2path=[]
    for fname in types:
        valpath_t2path.extend(glob(os.path.join(valpath_root, fname)))
    valpath_t2path = sorted(valpath_t2path)
    
    types = ('any_other_relevant_files.nii','*_pros_bas.nii') 
    valpath_prostate_masks=[]
    for fname in types:
        valpath_prostate_masks.extend(glob(os.path.join(valpath_root, fname)))
    valpath_prostate_masks = sorted(valpath_prostate_masks)
    
    #Load softmax and corresponding GT
    
    if opt.test_case.lower()=='nki_adc':
      nnunet_path = os.path.join(valpath_root,'Nnunet', 'ADC')
    elif opt.test_case.lower()=='nki_t2':
      nnunet_path = os.path.join(valpath_root,'Nnunet', 'T2')
    elif opt.test_case.lower()=='nki_t2adc' or opt.test_case.lower()=='nki_adct2' or opt.test_case.lower()=='nki_adc_t2' or opt.test_case.lower()=='nki_t2_adc':
      nnunet_path = os.path.join(valpath_root,'Nnunet', 'T2ADC')
    else:
      print('Did not get one of the expected input variables: nki_adc, nki_t2, nki_t2adc, nki_adct2, nki_adc_t2, nki_t2_adc')
    nnunet_pred_fold = os.path.join(nnunet_path,'Softmax')
    nnunet_gt_fold = os.path.join(nnunet_path,'Masks')
    
    types = ('any_other_relevant_files.nii','*.npz') 
    valpath_nnu_softmax=[]
    for fname in types:
        valpath_nnu_softmax.extend(glob(os.path.join(nnunet_pred_fold, fname)))
    valpath_nnu_softmax = sorted(valpath_nnu_softmax)
    
    types = ('any_other_relevant_files.nii','*_LES*.nii') 
    valpath_masks=[]
    for fname in types:
       valpath_masks.extend(glob(os.path.join(nnunet_gt_fold, fname)))
    valpath_masks = sorted(valpath_masks)
    
    impath=str(Path(valpath_adcpath[0]).parent)
    
    val_files = [{"t2w": t2w, "seg": seg, "img" : img, "prost": prost, "softmax": softmax} for t2w, seg, img, prost, softmax in zip(valpath_t2path, valpath_masks, valpath_adcpath, valpath_prostate_masks )] #last  n_val to validation
    
    csv_path=os.path.join(datadir,'testing_data_v3','NKI_testset_clin.csv ')
    ...
    valpath_nnu_softmax = os.path.join(datadir,'LINAC95_B_nii','Nnunet','Softmax')
    ...
    valpath_nnu_softmax = os.path.join(datadir,'MR_SIM34_BLfollowup_NII','Nnunet','Softmax')
...
print(f'Amount of val_files count: {len(val_files)}')
#==========================================================================
...
val_transforms = Compose(
    [
        LoadImaged(keys=["img","seg","prost", "t2w", "softmax"]),

        EnsureChannelFirstd(keys=["img", "seg","prost", "t2w", "softmax"]),
        Orientationd(keys=["img","seg","prost", "t2w" ,"softmax"], axcodes="RAS"),     
        
        ResampleToMatchd(keys=["t2w","seg","prost" ,"softmax"],
                              key_dst="img",
                              mode=("bilinear", "nearest", "nearest","bilinear")),
        Spacingd(keys=["img", "seg","prost", "t2w" ,"softmax"],
                      pixdim=(PD_in[0], PD_in[1], PD_in[2]),
                      mode=("bilinear", "nearest", "nearest","bilinear", "bilinear")),
        
        ScaleIntensityRangePercentilesd(keys=["img", "t2w"],lower=0,upper=99,b_min=-1.0,b_max=1.0,clip=True,channel_wise=True),
        CropForegroundd(keys=["img","seg","prost", "t2w", "softmax"], source_key= "prost", margin=[64,64,opt.extra_neg_slices + (opt.nslices - 1) / 2]),
        
        
        #RandSpatialCropd(keys=["img","t2w","seg"], roi_size=(128, 128, opt.nslices),random_size=False),,
        CenterSpatialCropd(keys=["img","t2w", "prost","seg" ,"softmax"], roi_size=[128, 128,-1]),
        EnsureTyped(keys=["img", "seg","prost", "t2w" ,"softmax"]),
...
with torch.no_grad(): # no grade calculation 
    ...
    for val_data in val_loader:
        step += 1
        adc, label_val,prostate,t2w, nnu_softmax = val_data["img"].to(device), val_data["seg"].to(device), val_data["prost"], val_data["t2w"].to(device), val_data["softmax"]
        ...
        nnu_softmax_name= nnu_softmax.meta['filename_or_obj'][0].split('/')[-1]
        
        print('ADC: %s T2w: %s GTV: %s Prostate: %s Softmax: %s' % (adc_name,t2_name,gtv_name,prostate_name, nnu_softmax_name))
        ...
        nnu_softmax_name=nnu_softmax.meta['filename_or_obj'][0]
        ...
        if adc.numel() != 0:
            ...
            val_data["pred"] = val_data["softmax"] 
            ...
            if  np.sum(gtv)>5:
                ...
                print(f"nnU-Net prediction shape before squeeze: {seg.shape}, dtype: {seg.dtype}, range: {seg.min():.3f}-{seg.max():.3f}")
                
                seg = np.array(seg)
                seg=np.squeeze(seg)
                
                print(f"nnU-Net prediction shape after squeeze: {seg.shape}, dtype: {seg.dtype}, range: {seg.min():.3f}-{seg.max():.3f}")
                
                softmax = np.array(seg) #Copy softmax before binarization
                ...
                DSC, FP, hd95, auc, gt_vol, pred_vol,gt_adc, pred_adc, labels=process_with_csv(img_name,gtv_name,opt.name,opt.test_case,model_arch_name, seg_filtered, gtv, adc_val, spacing_mm, fd_results, csv_path)
    ...
    q25_auc, q75_auc = np.nanpercentile(auc_list, [25, 75])
    ...
    print('Patient AUC - Median: %.2f, IQR: %.2f-%.2f' % (median_auc, q25_auc, q75_auc))
    ...
    fd_results_sum.write('name, median Lesion DSC, q25_Lesion_Dice, q75_Lesion_Dice, median whole volume DSC, q25 whole volume DSC, q75 whole volume DSC, median lesion hd95 (mm),  q25 lesion hd95 (mm), q75 lesion hd95 (mm), median AUC, q25 AUC, q75 AUC, precision, recall \n')
    fd_results_sum.write(opt.name + ',' + str(median_Lesion_Dice) +','+ str(q25_Lesion_Dice) +','+ str(q75_Lesion_Dice) + ',' + str(median_dice_3D) +','+ str(q25_dice_3D) +','+ str(q75_dice_3D)  + ',' + str(float(median_hd95)) +','+ str(float(q25_hd95)) +','+ str(float(q75_hd95)) + ',' + str(median_auc) + ',' + str(q25_auc) + ',' + str(q75_auc) + ',' + str(precision)+ ',' + str(recall)+'\n')
