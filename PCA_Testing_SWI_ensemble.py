...
class EnsembleInference:
    def __init__(self, model_configs, device):
        ...
        Args:
            model_configs: List of dictionaries, each containing:
                - 'opt': Model options or None to use default
                - 'path': Path to model weights
                - 'name': Optional model name for reference
        ...
        for config in model_configs:
            ...
            if 'opt' in config and config['opt'] is not None:
                model_opt = config['opt']
            else:
                model_opt = TrainOptions().parse()
                model_opt.isTrain = False
                model_opt.nchannels = model_opt.nslices * nmodalities
            
            # Get model path
            model_path = config['path']
            model_name = config.get('name', os.path.basename(model_path))
            model_weight = config.get('weight', 1.0)
            
            print(f'Loading model {model_name} from {model_path}')
            
            # Create and initialize model
            model = create_model(model_opt)
            model.load_MR_seg_A(model_path)
    ...
    def _prepare_model_for_eval(self, model):
        """Configure model for evaluation (BatchNorm settings, etc.)"""
    ...
    def inference(self, input_tensor, roi_size, sw_batch_size, overlap=0.66, combination_method='weighted_average'):
        """
        Perform ensemble inference with all loaded models.
        ...
        Args:
            ...
            combination_method: Method to combine predictions
                ...
                - 'weighted_average': Weighted average using model weights
        ...
        """
        predictions = []
        model_outputs = {}  # Store individual model outputs
        
        # Get predictions from all models
        with torch.no_grad(), autocast(enabled=True):
            for i, model in enumerate(self.models):
                print(f"Running inference with model: {self.model_names[i]}")
                ...
                model_outputs[self.model_names[i]] = pred
        
        # Combine predictions
        if combination_method == 'average':
            final_pred = self._average_combination(predictions)
        elif combination_method == 'weighted_average':
            final_pred = self._weighted_average_combination(predictions, self.weights)
        ...
        return final_pred, model_outputs
    
    def _average_combination(self, predictions):
        """Average the softmax outputs of all models"""
        return torch.mean(torch.stack(predictions), dim=0)
    
    def _weighted_average_combination(self, predictions, weights):
        """Apply weighted average to the softmax outputs"""
        weighted_preds = [pred * weight for pred, weight in zip(predictions, weights)]
        return torch.sum(torch.stack(weighted_preds), dim=0) / sum(weights)
    
    def _majority_vote(self, predictions):
        """Apply majority voting on binary predictions"""
        binary_preds = [pred >= 0.5 for pred in predictions]
        stacked = torch.stack(binary_preds, dim=0)
        votes = torch.sum(stacked, dim=0)
        return (votes >= (len(self.models) / 2)).float()
...
# types = ('ProstateX-????.nii.gz', 'MSK_MR_*_CTV.nii.gz', 'MSK_SBRT_*BL_CTV.nii*','CTV_*_.nii.gz') # the tuple of file types
# val_prost=[]
# for fname in types:
...
with torch.no_grad(): # no grade calculation 
    ...
    for val_data in val_loader:
        ...
        #print('  T2: %s ADC: %s GTV: %s Prostate: %s', %())
        ...
        #filters out less than 1mL
        #if (np.sum(gtv)*spacing_mm[0]*spacing_mm[1]*spacing_mm[2]/1000.0)<1.0:
        if  np.sum(gtv)>5:  
