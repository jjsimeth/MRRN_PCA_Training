...
types = ('ProstateX*_ep2d_diff_*.nii.gz', 'MSK_MR_*_ADC.nii.gz', 'MSK_SBRT_*BL_ADC.nii','ADC_*.nii.gz','MSK_DILVAR*_ADC.nii.gz') # the tuple of file types
val_images=[]
for fname in types:
   val_images.extend(glob(os.path.join(valpath, fname)))
val_images = sorted(val_images)

types = ('ProstateX*Finding*t2_tse_tra*_ROI.nii.gz', 'MSK_MR_*_GTV.nii.gz', 'MSK_SBRT_*BL_mask.nii','label_*.nii.gz','MSK_DILVAR*_T2GTV.nii.gz') # the tuple of file types
val_segs=[]
for fname in types:
...
types = ('ProstateX*_t2_tse*.nii.gz', 'MSK_MR_*T2w.nii.gz', 'MSK_SBRT_*BL_T2.nii','T2_*.nii.gz','MSK_DILVAR*_T2w.nii.gz') # the tuple of file types
val_images_t2w=[]
for fname in types:
